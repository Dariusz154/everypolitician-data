  ...
  class Membership < CSV
    ...
    def raw_table
      super.each do |r|
        ...
        r[:id] = r[:name].to_lower.gsub(/\s+/, '_') if r[:id].to_s.empty?
        r[:group_id] = r[:group].to_lower.gsub(/\s+/, '_') if r[:group_id].to_s.empty? && !r[:group].to_s.empty?
        r[:area_id] = r[:area].to_lower.gsub(/\s+/, '_') if r[:area_id].to_s.empty? && !r[:area].to_s.empty?
